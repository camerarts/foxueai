...
const VoiceStudio: React.FC = () => {
  ...
  // --- History Logic ---
  const addToHistory = async (blobUrl: string, txt: string, duration: number, isCloud = false) => {
  ...
  const downloadHistoryItem = async (item: HistoryItem) => {
      try {
          ...
          a.href = url;
          const fileName = `voice_${item.timestamp}_${item.provider}.mp3`;
          a.download = fileName;
  ...
  const handleGenerateSingle = async () => {
      ...
      try {
          ...
          addLog(`--> ✅ 生成成功 (${formatDuration(duration)})，准备播放`);
          ...
          await addToHistory(blobUrl, text, duration, false);
           ...
           <div className="max-w-4xl mx-auto h-full flex flex-col gap-6">
              ...
              <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4 sticky bottom-6">
                 ...
                 <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                    {/* Removed Preview Fragment Button */}
                 </div>
              </div>
           </div>
        </div>
                          ...
                          <div key={item.id} className="group bg-slate-50 hover:bg-white border border-slate-100 hover:border-violet-200 rounded-xl p-3 transition-all shadow-sm hover:shadow">
                              <div className="flex justify-between items-start mb-2">
                                  ...
                                  <div className="flex flex-col items-end">
                                      ...
                                      {item.duration && <span className="text-[9px] font-bold text-slate-300">{formatDuration(item.duration)}</span>}
                                  </div>
                              </div>
